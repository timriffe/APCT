...
A property of the framework that we propose is isotropy, i.e., that
the time units with respect to each time measure are equal.
...
used to generate Figurs~\ref{fig:apctTAL} and \ref{fig:apctAPC} are isotropic.
...
but we can think of it as shifting up and down through the space. At the peak,
near $\omega$ remaining years of life, the age axis of the plane is short, since
only the lowest chronological ages may live so long. The APC base plane at
thanatological age 0 is the largest because people of any chronological may die.
...
Some aspects of the framework we present are simple. For instance, it is easy to
...
the natural patterns of such conditions over the lifecourse. The measurement of
...
Some panel surveys with mortality followups already provide the requisite information, as do linkable registers that include items such as health measures or proxies and
