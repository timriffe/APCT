...
measures, and a full identity that relates the six of them. We provide
a topological overview of the diagrams that pertain to these
identities. The 3-d geometric representation of the full six-way
identity is proposed as a coordinate system to fully describe
temporal variation in demographic data.
...
which we refer to as ``thanatological age'' (T) in contrast to ``chronolgical age'' (A). The second index is death cohort (D), which groups all individuals (of different ages) dying in the same time period. Finally,
lifespan (L) is itself an index by which data may be structured. We therefore
have six time measures in total to relate. We call these measures of demographic
...
time itself.
This relationship is scalable to any time unit, but we describe it in terms of
...
necessary. We also describe the framework in terms of human lifespans, although
it applies in a more general sense to any durations observed over time.
...
Table~\ref{tab:dyads}.
The reader may also imagine this graph as a transparent 3d object, in which case the four faces become apparent. There are two intuitive ways to imagine the graph as 3d, either the vertex 4 is on top, and we gaze from a bird's-eye-view, or the vertex 4 is in the back, behind the other three vertices. Assume we
...
\begin{table}[h]
\centering
\caption{All informative triads,
...
areas of application. Above all, we believe in the pedagogical value of the
concepts and tools introduced in the following pages. 
