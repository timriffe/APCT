...
description of how any such temporal identities all arise from the notion of distinct events situated in time and the durations among them. These more general event-duration
foundations facilitate comparison of our proposed demographic time framework
with other temporal designs found in the literature, such as the disease duration space of \citet{brinks2014lexis},
...
% Added some motivation, clarifying that we don't provide an alternative to the Lexis diagram but an extension. --j
...
describe demographic time: chronological age (A), period (P), birth cohort (C), thanatological age or time-to-death (T), lifespan or age-at-death (L), and death cohort (D).
...
% I commented out the table header as the content is self-explanatory. I also changed the last column to a centered one. I did not check how it looks or if it compiles. --j

\begin{longtable}{m{0.15\textwidth}L{0.4\textwidth}C{0.2\textwidth}}%m{0.2\textwidth}}
  ...
  % \multicolumn{1}{c}{Cartesian representation} \\%& \multicolumn{1}{c}{Isotropic} \\
  \midrule
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% This paragraph sounded a bit aggressive to me. Like we had a personal quarrel with Lexis. I softened it a bit without changing the meaning. --j
...
% I tried to make the examples a tiny bit more specific. --j
...
patterns in other contexts, e.g. measurements on mother and fetus over the course of a pregnancy may vary by age of gestation, time until parturition and total length of the pregnancy; growth and reproduction patterns over age may be conditioned on the life-span of an organism.
...
% Removed reference to Appendix A. --j
...
% Pancho introduced the notion that the elements in p have to be chronologically ordered. I think this is too limiting. While its true that death always follows birth it is not true for other events or the notion of period. Entry into parenthood for some will be prior to marriage and for others after marriage. A certain period may be after diagnosis of disease or prior to it. The important part is just to have a way to name elements of p and d so that in a given Lexis space p2 is *always* period and d12 is *always* age, but the values the elements of p and d take must not be ordered at all. We draw the elements of p on the timeline ordered by index out of convenience, not out of nessecity. --j

\begin{definition} 
  Let $\boldsymbol{p}=(p_1,\ldots,p_n)^\top\in\mathbf{R}^n$ be a vector of $n$ events or points in time, with $n\geq2$. A corresponding vector of durations $\boldsymbol{d}\in\mathbf{R}^m$ is composed by elements of the form $d_{ij}=p_j-p_i$ for $i=1,\dots,n-1$, $j=2,\dots,n$ and $j>i$.
  \label{def:1}
\end{definition}
...
These results imply that given an arbitrary set of size $n\geq2$ of points in time, it is always possible to calculate the durations between any pair of these points. This calculation can be understood
...
% By arbitrarily swapping the rows of $\boldsymbol{X}$ it would be possible to obtain several other linear transformations and the corresponding vectors of durations different than $\boldsymbol{d}$

\begin{proposition}
 Given a vector of events $\boldsymbol{p}=(p_1,\ldots,p_n)^\top\in\mathbf{R}^n$, the corresponding vector of durations $\boldsymbol{d}\in\mathbf{R}^m$ is unique, irrespective of the sorting process.
\end{proposition}
...
This last proposition allows considering $\boldsymbol{X}$ as the matrix defining the linear transformation between points and durations. Given a vector $\boldsymbol{p}$ and the corresponding $\boldsymbol{d}=\boldsymbol{X}\times\boldsymbol{p}$, any differently sorted vector of durations would be obtained by swapping the rows of $\boldsymbol{X}$. Further, note that $\boldsymbol{X}$ does not have an inverse matrix, and therefore there is no linear transformation from durations to events. This is intuitively straightforward if one thinks that two vectors of events can yield the same vector of durations, or in other words, that a particular vector of durations can come from infinite different vectors of points in time. For instance, using $\boldsymbol{X}$, the vectors of events $\boldsymbol{p^1}=(1,2,3)$ and $\boldsymbol{p^2}=(2,3,4)$ both yield $\boldsymbol{d}=(1,2,1)$. In relation with the six time measures discussed here, note that the events CPD yield TAL, but TAL does not yield CPD.
...
the main timeline as curly braces labelled with $d_{12} \ldots d_{n-1,n}$.

The joint relationship between durations and events is more explicit and more
compact in a graph representation. The total number of time measures implied by a set of $n$ events and the corresponding durations is
therefore $n+m=n+n(n-1)/2=n(n+1)/2$, which yields a $n+1$-polytope. % A set of time measures derived in this way, $\boldsymbol{g} = (\boldsymbol{p},\boldsymbol{d})\in\mathbf{R}^{n(n+1)/2}$, forms an identity. In the same way that $\boldsymbol{d}$ derives from $\boldsymbol{p}$, there are $(n+1)^{(n-1)}$ ways to complete $\boldsymbol{g}$ from a size-$n$ set of potentially mixed durations and events. Proofs for these statements are found in Appendix~\ref{app:A}. 
%The middle column of Table~\ref{tab:timelines}, labelled \textit{graph} consists in the simplest graph representation of the event-duration timeline, with $n$ red vertices for each of the events in $\boldsymbol{p}$. A full connection of the graph yields $m$ edges for each of the possible durations in $\boldsymbol{d}$. This graph representation loses the linear time analogy of the timeline, but makes the depdenency of $\boldsymbol{d}$ on $\boldsymbol{p}$, and other interdependencies between time measures more explicit.
The right column of Table~\ref{tab:timelines}
 draws the corresponding graph with a total of $n+1$ vertices and $n+m$ edges
for the elements of both $\boldsymbol{d}$ and $\boldsymbol{p}$. All
events of $\boldsymbol{p}$ connect to a single vertex. Event edges are indicated
...
\begin{equation}
d_{12} = p_2 - p_1   \quad\quad.
\end{equation}

Interpreting $d_{12}$ as \emph{age}, $p_2$ as \emph{period}, and $p_1$ as
\emph{birth cohort} yields the APC identity. The Lexis surface is constructed via change of basis of the point-space $P$, featuring basis vectors $(p_1, p_2)$, to the point-duration space $M$, featuring basis vectors ($p_2, d_{12}$).
...
Let $\boldsymbol{p}$ have three elements, as in the second row of
...
Interpreting $p_1$ as \textit{birth cohort}, $p_2$ as \textit{marriage cohort} and $p_3$ as
 \textit{separation cohort} yields the durations $d_{12}$ as \textit{age at
 marriage}, $d_{13}$ as \textit{age at separation}, and $d_{23}$ as \textit{duration
 of marriage}. Lexis' ``marriage space'' $M$ is reconstructed by a change of basis of $P\subseteq\mathbf{R}^3\to M\subseteq\mathbf{R}^3$, with the new orthogonal basis formed by $(p_1, d_{12}, d_{23})$.

\paragraph{Example 3: Adding death cohort to the Lexis surface}

As in Example 2 we start with a three element vector
$\boldsymbol{p}$ yielding the very same identities as in equation set
\eqref{eq:p3} and the second row of Table~\ref{tab:timelines}, but with different interpretations.
Interpreting $p_1$ as \textit{birth cohort}, $p_2$ as \textit{period} and $p_3$ as \textit{death cohort} yields the durations $d_{12}$ as \textit{chronological age},
$d_{13}$ as \textit{lifespan}, and $d_{23}$ as \textit{time-to-death}. This
vector space contains the Lexis surface as a sub-space, as well as the
other planes presented in Sect.~\ref{sec:triads}. We return to this identity
...
of $\boldsymbol{d}$: $d_{12}$ is \textit{chronological age}, $d_{13}$ is \textit{age at
diagnosis}, $d_{23}$ is \textit{time to/since diagnosis},
% I think this is more clear. --j
\footnote{For points in time past the time at diagnosis $d_{23}$ becomes negative and can be interpreted at time since diagnosis.}
$d_{14}$ is \textit{lifespan}, $d_{24}$ is \textit{time-to-death},
and $d_{34}$ is duration of illness (an irreversible state).

Note that unlike in the other examples the actual vector-space of demographic time as shown in \citet{brinks2014lexis} figure 2 only identifies a \emph{subset} of the times-scales discussed by the authors, namely age ($z$-axis), period ($x$-axis), birth cohort (implied by a linear combination of age and period) and duration of disease ($y$ axis). No further time scales can be derived from this. Instead, the additional events and durations (death cohort, timing at diagnosis and duration of disease) are introduced as \emph{markings} within the three dimensional vector space, just like one would mark \emph{specific} life-lines on a Lexis-diagram without regarding \emph{all possible} life-lines in doing so.

\FloatBarrier
\section{A tetrahedron relates the six demographic time measures}
% TODO only tetrahedral graph: remove refs to other one
...
redrawn in Fig.~\ref{fig:sq} with edges labelled by the six time measures
...
triad identities. In Fig.~\ref{fig:tettet} each triangle also is the
...
imagine the tetrahedral graph as the wire-frame of a 3d
tetrahedron --- as the 3d edge structure of the tetrahedron platonic solid.

\begin{figure}[h!]
\centering
\caption{Graphs of demographic time hexad identity, with edges labelled by the
six time indices.}
\label{fig:tet}
\begin{subfigure}{.48\textwidth}
...
\label{fig:tettet}
\includegraphics[width=3in]{Figures/TetraHedronEdgesOnly.pdf}% {Figures/TetraHedronEdgesOnly.pdf}
\end{subfigure}
\end{figure}

For exposition, imagine that the middle vertex of Fig.~\ref{fig:tettet} is the
...
system to a three dimensional space follows \vt{def 4.1
and prop 4.2}.
...
the four diagrams presented in sections~\ref{sec:apc} to~\ref{sec:lcd}. A full
...
Let us take the example of self-reported health (SRH). 
 The data come from the RAND Corporation version of the US Health and Retirement Study
...
Further methodological details are given by \citet{riffe2016ttd}. 
...
When weighted by lifelines, the marginal chronological age pattern of SRH, measured with the ``Sullivan curve'' \citep{Sullivan1970}, would show an increasing
tendency over age, in agreement with common expectations.
...
prediction leads to pessimistic morbidity scenarios when mortality improvements are foreseen, and it undermines health expectancy comparisons between groups with different mortality \citep{vanRaalte2015HLE}.
...
Sullivan method and assuming the 1980 chronological age
pattern of poor SRH.\footnote{The 1980 chronological age pattern of poor SRH is
based on the 1980 stationary population and the same fixed time-to-death
...
demographic time: chronological age (A), period (P), birth cohort (C), time-to-death (T), death
cohort (D), and individual lifespan (L). In Sect.~\ref{sec:dyads2diagrams} we show how
combinations of these time measures imply four triad identities, each of which consists in simple linear relationship between its three
...
%in Sect.~\ref{sec:gen}. We explain how durations depend on events (and not
...
%\end{proof}
%\vt{need to be clear about what ``identity size'' means. Here it's based on
%vertex count, but elsewhere based on edge count}

%For example, from the tetrahedral graph in Fig.~\ref{fig:tettet}, we may
