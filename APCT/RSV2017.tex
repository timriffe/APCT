...
ten triad identies, respectively. We call this a temporal plane graph because
the triangle resulting from any given triad sub-identity can be extended over
...
redrawn in Figure~\ref{fig:tet} labelled with the six time indices. \vt{include square
version of the same.}
...
\begin{appendices}
\section{The event duration transformation proofs}
\label{app:A}

As described in Section~\ref{sec:gen}, $\textbf{p}$ is a length-$n$ vector of
event times. Consider the set $P$ of all possible choices of $n$ points in time.
...
durations between any pair of these points. This calculation can be understood as a linear transformation of the \textit{point-space} $P$ to the \textit{duration-space} $D$.
\end{theorem}
...
\begin{theorem} *(Work in progess)* Something that states that you can transform $P^n \leftrightarrow M^n$ where the basis vectors of $M$ are a mixture of point dimensions and duration dimensions.
\end{theorem}
...
\begin{theorem} There are $b = (n+1)^{(n-1)}$ many ways to
choose $n$ elements out of $\textbf{g}$ whoose linear combination yields the
