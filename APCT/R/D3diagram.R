...
for (z in 0:(length(z1)-1)){
    for (i in 1:length(x1)){
		rgl.linestrips(c(x1[i],x2[i]-10*z),c(0,100-10*z),c(10*z,10*z),col="black",alpha = .7)
	}
}
...
for (z in 0:(length(z1)-1)){
	for (i in 1:(length(x1)-z)){
		rgl.linestrips(c(x1[i],x2[i]),c(y1[i],y2[i]),c(10*z,10*z),col="red",alpha = .7)
	}
}
...
for (z in 0:(zl-1)){
	...
	for (i in 1:(length(x1)-z)){
		...
		rgl.linestrips(c(x1[i],x2[i]),c(y1[i],y2[i]),c(10*z,10*z),col="green",alpha = .7)
	}
}
...
for (z in zl:0){
	for (y in 0:10){
		for (x in x1){
			#y22 <- y2-10*z
			#y22 <- ifelse(sign(y22) < 0, 0, y22)
			rgl.linestrips(c(x+y*10,x+y*10),c(y*10,y*10),c(0,max(10*z - 10*y,0)),col="blue",alpha = .5)
		}
	}
}
...
for (iz in inc){
	for (i in 1:length(x1)){
		rgl.linestrips(c(x1[i],x2[i]+iz),c(0,iz),c(iz,0),col="magenta", alpha = .7)
	}
}
...
rgl.triangles(x=c(1900,2020,1900),y=c(0,120,0),z=c(0,0,120),col="magenta",alpha = .3)
rgl.linestrips(c(2000,2025),c(100,125),c(0,0),color = "magenta",alpha = .7)
text3d(2030,130,0, text = "1900 cohort ATL", col="black", cex = 1.1,alpha = 1) 
# Add ATL semitransparent triangle for 1900 period
rgl.triangles(x=c(1920,1920,1920),y=c(0,120,0),z=c(0,0,120),col="green",alpha = .3)
...
for (i in 1:11){
	rgl.linestrips(c(x1,x2[i]),c(y1[i],y2[i]),c(0,0),color = "red",alpha = 1)
}
...
# do the same for period on right side:
x1 <- x2 <- seq(2000,2100,by=10)
y1 <- 0;y2 <- seq(0,100,by=10)
for (i in 1:11){
	rgl.linestrips(c(x1[i],x2[i]),c(y1,y2[i]),c(0,0),color = "green",alpha = 1)
}

# ====================================
# add planes for start and end of observation (1900,2000), indicating past and future
rgl.quads(x = c(1900,1900,1900,1900), y = c(-10,110,110,-10), z = c(-10,-10,110,110), col = gray(.5),alpha=.1 )
rgl.quads(x = c(2000,2000,2000,2000), y = c(-10,110,110,-10), z = c(-10,-10,110,110), col = gray(.5),alpha=.1 )
...
y <- max(c(diff(par3d("bbox")[3:4]),diff(par3d("bbox")[5:6])))
decorate3d(xlim=c(1800,2100), ylim=c(0,100), zlim=c(0,100),aspect=c(x,y,y),box = FALSE, axes = FALSE, xlab = "",ylab = "", zlab = "")
