...
the main timeline as curly braces labelled with $d_{1,2} \ldots d_{n-1,n}$.

% TR: removed 'vertices have no direct time measure meaning', because
% I now think they can be interpreted also in the n+1 graph, but no focus on
% that.
...
therefore $n+m=n+n(n-1)/2=n(n+1)/2$, which yields a $n+1$-polytope. The right
column of Table~\ref{tab:timelines} draws the corresponding graph with a total of $n+1$ vertices and $n+m$ edges
...
\emph{birth cohort} yields the APC identity. The Lexis surface is constructed
via change of basis of the point-space $P$, featuring basis vectors $(p_1, p_2)$, to the point-duration space $M$, featuring basis vectors ($p_2, d_{1,2}$).
...
Interpreting $p_1$ as \textit{birth cohort}, $p_2$ as \textit{marriage cohort} and $p_3$ as
 ...
 \textit{duration of marriage}. Lexis' ``marriage space'' $M$ is reconstructed
 by a change of basis of $P\subseteq\mathbf{R}^3\to M\subseteq\mathbf{R}^3$,
 with the new orthogonal basis formed by $(p_1, d_{1,2}, d_{2,3})$.
...
\textit{time to/since diagnosis},
% I think this is more clear. --j
\footnote{For points in time past the time at diagnosis $d_{2,3}$ becomes
...
time shown in \citet{brinks2014lexis} figure 2 only identifies a
...
Although lifelines in this depction only begin to ascend into the disease
duration axis at the time of disease diagnosis, this event time measure is not
ascribed to an axis per se, and no further time scales can be derived from
the three axes drawn.
Instead, the additional events and durations (death cohort, timing of diagnosis
and duration of disease) are introduced as \emph{markings} within the three
dimensional vector space, just like one would mark \emph{specific} life-lines on
a Lexis-diagram without regarding \emph{all possible} life-lines in doing so.
The four-dimensional vector-space can be thought of as the larger context within
