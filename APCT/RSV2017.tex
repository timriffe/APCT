...
%The concepts of thanatological age and death cohorts are likely less familiar
% to
...
%or residual life, but we prefer the term thanatological age. Age in this
% general sense marks a position on a %lifeline with respect to one of its endpoints. Chronological age and thanatological age are in this way
...
%demography the grouping characteristic is often a combination of place and
% time, such as
...
%\nth{37} class of entering students since the start of the program. These
%concepts are analogous to the ideas %of
%birth and death cohorts we use here, but we do not often refer to the deaths of
% a given year as a death cohort.
%In the time preceding death, the members of a given death cohort have much in
% common, despite
...
%the case, for instance, in biodemographic studies in which wild animals with
% unknown ages are captured and then followed-up until death
%\citep{muller2004demographic, muller2007survival}, but it can be
%generalized to other kinds of human and non-human demographic data with a
%similar structure. For example, in the Barcelona Historical Marriage
%Database, which collects information about marriage licenses of Barcelona
...
%for the representation of lifelines preceding deaths from infectious or
% acquired conditions, when the time of infection or acquisition remains unknown.
...
% TR 12-4-2017 removed to shorten paper
...
%[^1]: Equivalently $\textbf{d}$ can be expressed as
% $\text{vech}(\textbf{p}\times \textbf{1}_n^\textsf{T} - \textbf{1}_n \times \textbf{p}^\textsf{T})$. For ease of notation we use double indices to index each element of the vector $\textbf{d}$. The indices $ij$ of the $k^\text{th}$ element of $\textbf{d}$ are given by $d_{ij} = d_k = p_{i=\left\lfloor \frac {1}{2}+\sqrt{2k}\right\rfloor+1} - p_{j=k-C\left(\left\lfloor \frac{1+\sqrt{8k}}{2}\right\rfloor\right)}$ with $C[f(k)]$ being the binomial coefficient $\left(\begin{matrix} f(k)\\2\end{matrix}\right)$.
%[^2]: $\textbf{I}_n$ is the $n \times n$ identity matrix; $\textbf{1}_n$ is a
% vector with $n$ elements where each element is 1; $\textbf{0}_{m \times n}$ is a $m \times n$ matrix where each element is 0. For notational convenience we allow for a matrix with 0 columns, written $\Big(\textbf{0}_{n \times 0}\Big)$.
...
\label{fig:sq}
% switched to edges only, since numbers not referred to
...
%\vt{This section to be replaced with general intro, but language from here can
%move up possibly}

%A geometrical analogy is pertinent at this point. Any pair of intersecting
% edges of the tetrahedron may be interpreted as two vectors $\vec{u}$ and $\vec{v}$
%that determine a 2-dimensional plane in a Cartesian 3-dimensional
%space ($\mathbb{R}^3$).\footnote{A 2d plane in a 3d space is determined by two
% linearly independent vectors (with different direction) and a point, but the inclusion of a point is not
%necessary for the intuitive analogy that we describe here.} Therefore, any
% third vector $\vec{w}$ of that plane can be expressed as a linear combination of
%$\vec{u}$ and $\vec{v}$ (formally, $\vec{w}=\alpha\vec{u}+\beta\vec{v}$ for
% some $\alpha, \beta \in \mathbb{R}$), which is usually described by saying that $\vec{w}$ is linearly dependent on $\vec{u}$ and
%$\vec{v}$.
...
%``depends'' on C and A, and $C=P-A$ ``depends'' on P and A. Given that the
% faces of the tetrahedron represent the triad identities, any pair of intersecting edges has the same property: The third edge located in the same face of the tetrahedron can be determined by the first two by a simple linear relationship.
...
%Once a 2d-plane is defined in $\mathbb{R}^3$, an additional vector may be
% sufficient to cover a 3d-space. Nonetheless, this third vector needs to be linearly independent
...
%the indices of the APC plane; A, C and P are not sufficient to determine a
...
%hexad identity or, using an analogous terminology, covers the whole ``space''
% of demographic indices presented here. Graphically, this is equivalent to
...
% TR 12-4-2017 removed to shorten paper (and reviewer 2 found this section
% unclear)
...
%available from many different survey sources, and it is familiar to many
% researchers. There are many known pitfalls to this particular variable that already make it difficult
...
%pattern of variation that it shows, which we use to make our more general
% point.
...
%the six demographic time measures has never appeared in the literature.
% However, \citet{lexis1875einleitung} describes an analogous hexad relationship between,
%birth cohort, age at marriage, age at separation (death or widowhood), marriage
% cohort, separation cohort, and duration of marriage. The hexad relationship between these six
...
%two-dimensional diagrams, with values of the orthogonal dimension held
% constant.
...
%illustrated with complex perspective line drawings (see Lexis' Figures 5 and
% 6).
%These figures (based on cubes rather than tetrahedra--octahedra) and the
% portion of the text that discusses these relationships has unfortunately been largely
...
%identity are not transportable, but perhaps the specificity of this identity
%has made it unwieldy in practice, or difficult to conform with data. Adding an
...
%pencil notion from \citet{francis1996visualization}. Lexis pencils are a method
% of representing limited numbers of multistate lifelines in three dimensional space, where
...
% years of life and lifeline trajectories were realigned to descend toward zero remaining years
%of life (or some rotation thereof), and sorted by length of life.

\section{Discussion}

In this paper we first present a series of four Cartesian diagrams based on
combinations of six different measures of demographic time: chronological age,
period, birth cohort, time to death, death cohort, and individual lifespan.
These diagrams include the familiar Lexis diagram (APC), but also three either new or uncommon diagrams. The TPD
diagram relates time of death, period, and time until death, and is therefore a
sort of dual to the Lexis diagram. The TAL diagram relates the three duration
measures of time to death, chronological age, and lifespan, and it may be useful
for analyzing patterns that vary over the lifecourse and/or by length of life.
The CDL diagram relates time of birth with time of death and lifespan, and it
may be useful for analyzing patterns that vary over time and by length of life,
but not necessarily over the lifecourse. We describe these four diagrams as
temporal plane extensions of their underlying triad identities. Each triad
identity consists in simple linear relationship between its three constituent
time measures.

In Section~\ref{sec:gen} we digress to present a more general
event-duration framework. These general terms allow us to present the
demographic time hexad identity as a special case in Section~\ref{sec:examples}.
We compare this identity with other relatively complicated temporal
relationships in the literature, such as the \citet{lexis1875einleitung}
marriage identity. In Section~\ref{sec:tetrahedron} we describe how the graph
of the demographic time identity is also the graph of a tetrahedron. As a
three-dimensional solid, the tetrahedron forms the basis of the three
dimensional extension of the demograhic time identity. In
Section~\ref{sec:diagram} we present a diagram rendering of the demographic
time identity.
...
\begin{proof}
The follows by noting that the $n$ event-labelled edges in $\textbf{K}$ connect
