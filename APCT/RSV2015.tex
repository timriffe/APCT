...
system for processes that vary over the lifecourse. Since the
lifecourse belongs to the cohort perspective, it is best to think of the TAL
...
The APC, TPD, TAL, and CDL planes can be conceived of as
...
TAL diagram, but it seems likely one will have arisen in the field of
biology, albeit with no relation to the present discourse. We suppose
that CDL diagrams of any kind are yet-unknown.

A simultaneous juxtaposition of the six demographic time measures has certainly
also never appeared in the literature, either as a visual diagnostic or in any line of
...
\section*{Motivation, post hoc.}
The relationships implied by the hexad of demographic time measures described
here are valid irrespective of their usefulness. The present authors are
unable to provide an exhaustive overview of the areas in which this
coordinate system may be applied. Further implications are deserving of further
inquiry by demographers and others. The only application that has received
attention at this time is the measurement and modelling of
late-life health.
For instance, faithful measurement of potential morbidity compression is only
possible in a fully defined hexad coordinate system. If this task is undertaken
in an APC-only framework, compositional and lifetable heterogeneity can and do
obfuscate inquiry and misdirect conclusions. This particular case will be
described elsewhere. It is likely the case that failure to account for
appropriate temporal variation in demographic models has also led to
misunderstandings.

The facts that we know about
empirical regularity in science are based in the first instance on
visualization, which for demographers means data visualization. If for the rest
of science, advancement was enabled in the form of better optics, this is the
role that the present APCTDL model pretends to assume for demography. 
Ironically, although this work provides no data visualizations, per se,
visualization is likely to be the first step in recognizing which are the
pertinent forms of variation in demographic phenomena that vary over the
lifecourse. Some of the standard age patterns that demography has documented may
be due for a revision due to the fact that thanatological or lifespan variation
was omitted in their description. Some conclusions in the literature may be
vulnerable to reversal, it's hard to say. We are but simple lens grinders, and
we offer no substantive arguments at this time.
