...
The \sout{so-called} \textbf{well-known?} Lexis diagram relates the chronological age (A), period (P),
...
measures \textbf{PANCHO: that generates a 3-dimensional space? ATC is a triad that just creats a plane, not a 3d-space}
...
Lexis plane, the Lexis diagram. If we know that Mindel turned 50 on the \nth{21}
of May, 1963, then we also also can derive that she was born on the \nth{21} of
...
therefore opens the door to new classes of comparisons, models of understanding, and discovery,
...
Note to Pancho and Jonas:
This section will contain Jonas' table of all 15 dyadic relationships, each with
a right and isotropic rendition. Describe graphing conventions, and walk through one line
...
We'll use the first names, birth and death dates of demographers.
Some suggestions to include those in Table~\ref{names}. You can get more
inspiration from \protect\url{http://www-personal.umich.edu/~eklanche/digdemog/index.html}
Maybe an idea would be to allow for a ``broken'' table, with the textual passage
between rows, similar to using intertext in math environment?
\end{tcolorbox}

 \begin{table}
\centering
\caption{Pick and choose (first) names from here for in-text references and
examples of derived measures (and uninformative measures). This table won't be in the final
text. These dates came from laborious web
detectivry. } 
\label{names}

\small{
\begin{tabular}{lllllllll}
Sex	&First	&Last	&BD	&BM	&BY	&DD	&DM	&DY\\
f	&Margaret	&Merrell	&3	&Dec.	&1900	&21	&Dec.	&1995\\
f	&Helen	&Abbey	&1	&Sep.	&1915	&4	&March	&2001\\
f	&Irene	&Taeuber	&25	&Dec.	&1906	&24	&Feb.	&1974\\
f	&\`{A}ngels	&Torrents		&\_&	\_&1940	&24	&June	&2004\\
m	&Pascal	&Whelpton	&22	&Feb.	&1893	&\_&	\_	&1964\\
f	&Mindel	&Sheps	&20	&May	&1913	&15	&Jan.	&1973\\
f	&Harriet	&Presser	& \_&	\_	&1936	&1	&May	&2012
\end{tabular}
}
\end{table}
...
tetrahedron has this quality. The South face TDP has no access to \vt{1}.
...
the tetrahedron are stuck in ``flatland''. However, there are \sout{twenty} $\binom{6}{3}=20$ ways \sout{in
total} to choose three time indices \sout{from our total}\textbf{out} of six, and the four
above-named triads are the only \sout{four of these}\textbf{ones} that will not yield the full 3d
...
BEGIN NEW PANCHO (I suggest to replace the following paragraph, or at least to introduce this geometrical perspective somewhere in the text)
\smallskip
...
A geometrical analogy seems pertinent at this point. Any pair of intersecting edges of the tetrahedron may be interpreted as two vectors $\vec{u}$ and $\vec{v}$ that determine a 2-dimensional plane in a 3-dimensional space.\footnote{Technically, a 2d plane in a 3d space is determined by two vectors of different direction and a point, but the inclusion of a point is not necessary for the intuitive analogy that we aim to present here.} Therefore, any third vector $\vec{w}$ of that plane could be expressed as a linear combination of $\vec{u}$ and $\vec{v}$ ($\vec{w}=\alpha\vec{u}+\beta\vec{v}$), which is usually referred as saying that $\vec{w}$ would be linearly dependent on $\vec{u}$ and $\vec{v}$. A similar property can be derived from the information contained in the tetrahedron: say we have information on year of birth $C$ and period $P$. Clearly, $C$ and $P$ are sufficient to determine the chronological age $A$, given that  $A=P-C$. That is, $A$ is a linear combination of $C$ and $P$ and it ``depends'' on them because they all belong to the same $APC$ plane (see Figure~\ref{fig:tet4vert1} !!THE FIGURE SHOULD BE MODIFIED). Analogously, $P=C+A$ ``depends'' on $C$ and $A$, and $C=P-A$ ``depends'' on $P$ and $A$. It can be easily verified that any pair of intersecting edges have the same propriety: the third edge located in the same face of the tetrahedron can be determined by the first two by a simple linear relationship.

From a geometrical perspective, once a 2d-plane is defined, an additional vector may be sufficient to cover the whole 3d-space. Nonetheless, this third vector needs to be linearly independent of any pair of vectors of that 2d plane---that is, it cannot be expressed as a linear combination of any two vectors on that plane. Again, an analogous propriety can be observed in the tetrahedron: say we only have information about the indices of the $APC$ plain. $A$, $C$ and $P$ are not sufficient to determine neither the thanatological age $T$, nor the death cohort $D$, nor the lifespan $L$ (the three indices that do not belong to the $APC$ plane). So, $T$, $D$ and $L$ are ``independent'' of the overall information that can be extracted from the $APC$ plane.

However, if two indices of the $APC$ plane are known (the third one would be unnecessary as it could be derived from the other two), the additional information provided by any of these three ``independent'' indices $T$, $D$ or $L$ would be sufficient to cover the whole tetrahedron. For example, suppose we have information about thanatological age $T$ in addtion to $C$ and $P$, then $A=P-C$, $D=P+T$ and $L=T+A=T+P-C$. Hence, as with vectors in a 3d space, any triad of indices that are independent of each other---that is, none of them can be expressed as the sum or the subtraction of the other two---generates a full hexad identity or, using an analogous terminology, covers the whole ``space'' of demographic indices presented in this paper. Graphically, this is equivalent to chose any combination of three indices that do not belong to the same face of the tetrahedron in Fig.~\ref{fig:tet}.
