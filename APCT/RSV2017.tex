...
complementary, duals. Birth and death cohorts are a similar kind of dual.
...
based on the AC dyad are variously referred to as Type III rates
...
diagrams in Section~\ref{sec:triads}. The dimensionality of the extended diagram of a given identity follows from the
...
For exposition, imagine that the middle vertex of Figure~\ref{fig:tettet} is the
...
brief case study based this coordinate system. We aim to demonstrate the
...
etc). These follow the coordinates of the TAL diagram in Figure~\ref{fig:TAL}.
...
When weighted by lifelines, the marginal chronological age pattern of SRH, as
measured here (the Sullivan curve, \citep{Sullivan1970}), would show an increasing
...
that seen in Figure~\ref{fig:poorsrh} would predict improvements in the marginal
...
Figure~\ref{fig:poorsrh} surfaces is valid for them.
...
\citep{vanRaalte2015HLE}. We then calculate the resulting healthy and unhealthy
...
data structure, we demonstrate that the choice of age pattern when calculating
prevalence-based measures of healthy life expectancy can have a non-trivial impact on the result.

\vt{from here down needs to have some clear structure still}
...
in one of the diagrams from Section~\ref{sec:dyads2diagrams}, similar to that
shown in Figure~\ref{fig:poorsrh} on the basis of the TAL diagram.
...
identity-planes. Other more complicated temporal designs based on the
event-duration framework in Section~\ref{sec:gen} and Appendix~\ref{app:A} are
also possible.
