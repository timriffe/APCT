...
In the case of dyads that imply a derived measure, another logical mapping is
to translate to an $(x,y)$ coordinate that forces 60$^\circ$ angles between the
three measures. Such a mapping ensures that the
spatial units are equal for the three measures, and we therefore refer to it as the isotropic mapping.
The isotropic mapping is comparable to using a ternary coordinate system, which we
do not discuss. It does not make sense to provide an isotropic mapping of an
uninformative dyad. 
...
temporal planes comes from a data visualization perspective, where it may be
