...
We describe a three-dimensional relationship between six different measures of demographic time: chronological age, time to death, lifespan, time of birth, time of death, and period. We describe four identities among subsets of these six measures, and a full identity that relates the six of them. One of these identities is the age-period-cohort identity, while the other three are relatively novel. We
...
third, which means that any dyad from this set is informative. 
...
birth and/or death. For this reason we call this a framework of demographic
time.
...
or residual lifespan, but we prefer the term thanatological age
\citep{riffe2015force}. Age in this general sense marks a position on a lifeline with respect to one of its
endpoints.
...
These concepts are analogous to the ideas of birth and death cohorts we use
...
is comparable to using ternary or barycentric coordinate systems, which we do
not discuss. 
...
base and vertex four on top. The same graph could be composed in four basic
...
recreate the full tetrahedron (hexad identity), and so we call them
\textit{informative triads}.
...
As mentioned, a compressed A compressed TPD diagram has thus far only appeared
...
inquiry or argumentation.\footnote{There are, however, some novel temporal
...
represent series of cross-sections. The latter are much easier to implement. In
the present case, cross-sections could be any of the four planes (triad identities) held constant for a given value of the
dimension that is ``sliced through''. For example, a set of TAL contour
plots (similar to ``Lexis surfaces''), each for a different birth cohort,
probably as a series of small multiples \citep{tufte1983visual} is one way to visualize data in the construct
we propose. This is the strategy we follow in the application example.
...
be inversely proportional to the number of life lines in the plot, and the plot
device would need to be flexible and interactive in order to relate life
lines to the various demographic time measures. 

A set of life lines squeezed together and sorted by lifespan becomes a survival
curve. Event charts \citep{goldman1992eventcharts,lee2000extensions}, or
visually richer event history graphs \citep{carey1998simple,dubin2001event} have
been developed as a way to display within-individual sequences under survival
...
grouping or smoothing techniques proposed by \citet{piccarreta2012graphical} and
\citet{fasang2013visualizing}. 
...
This is a larger statistical question about the prediction of remaining
lifetime for living individuals, and it is not a problem peculiar to
visualization. One can either opt to omit living individuals from the space, as
their coordinates are not yet determined. This is the course we take in the
application example. Often one would like to predict for these
live-censored individuals, either for the sake of making a more
relevant statement about an entire population stock \citep{wolf2015disability},
or for the sake of completing an event history graph \citep{dubin2001event} so
as to better communicate a distribution. In this case
...
enable discovery, and to better inform demographic methods. Such a set of
derivatives would demonstrate the usefulness of this coordinate system, which
might otherwise not be evident to the reader. We therefore give a shematic
overview of a real process of scientific inquiry that was based this coordinate
system, and that would not have arisen without it. In short, we explain how one
can come to the conclusion that many health expectancies cannot be projected, compared between groups, or compared over time without taking into account the
time-to-death process and changing mortality.

There are three steps in our empirical inquiry. The first step is to visualize
variables on health outcomes using this coordinate system. The second step is to
determine the primary time measures over which health outcomes vary. Under the
...
for time-to-death processes. Using standardization, one can show that estimates
of health expecancies are very biased when mortality has been or will-be
changing. We know that mortality has been changeing, and we assume that
mortality will be changeing, and so we conclude that comparisons of health
expecancies
...
Variables that change over the life course, such as health variables,
``fill'' the temporal space represented by e.g., Figure~\ref{fig:apctTAL}.
Visualization techniques may therefore borrow from those used in other
disciplines for three-dimensional composites, such as 
