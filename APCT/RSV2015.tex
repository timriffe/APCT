...
respective face is repeated in equal intervals, we have an isotropic 3d space.
...
cross-sectional planes are placed in sequence in a perspective drawing.\footnote{The coordinates used to render Figures~\ref{fig:apctTAL} and \ref{fig:apctAPC} are isotropic.
...
25-year intervals. The base of this figure is the APC plane, drawn for
thanatological age 0. Each of the TAL planes sits atop a single birth cohort
line from the familiar APC plane that makes up the base of the figure.
...
information, we only know that this infant is located somewhere on the left edge
of the front triangular TAL plane, which is depicted as an axis labelled
for thanatological ages 0 to $\omega$. If this infant is destined to die in the
year 2100, then the vertical position at birth will be at the intersection of
chronological age 0 and the left axis tick for thanatological age 100. This
person's lifecourse stays on the 100 lifespan line (labelled, orange), descending over time. Point A marks the midpoint in life for this individual, at chronological age 50 (red line), and thanatological
age 50 (green line). If another APC plane were drawn through thanatological age
50, we would see that point A is in the year 2050. Since all individuals born in
the year 2000 complete the same age in the same year, we can
also recuperate the year for point A by following the chronological age 50 line
(red) down to where it meets the blue line for the year 2050. The lifeline descends downward toward the APC plane for thanatological age 0 at chronological age 100, meeting the year 2100, which determines the death cohort, still within the birth cohort

Any of the other diagrams (APC, TPD, LCD) could be used to link together the
sequence of birth-cohort TAL planes. This view emphasizes how the TAL shifts
over time. Since the TAL diagram describes the lifecourse, a sequence of TAL
diagrams may be used to describe how the lifecourse changes over time.
This is the view for which we describe an application in a later section.
...
\texttt{text to locate somewhere}
...
settings, and in man-made structures.
...
We have been unable to locate an example in the literature of a
 compressed TAL or cross-sectional TPD diagrams, though it seems plausible that the former
will have arisen. We suppose
...
duration of marriage. Lexis describes cross-sections in the
same sense (\textit{Schnitte}), denoting two-dimensional diagrams, with values
of the orthogonal dimension held constant. It is interesting to note that the 

Lexis' proposal adds complexity
because 

Indeed, to add a single time measure to the basic hexad identity already
expands the relationship in more directions than can be easily reprented in three dimensional space, unless some measures are dropped.
