...
We assume that the vector of events $\boldsymbol{p}$ is chronological ordered for convention, to avoid negative durations. For instance, if $p_1>p_2$ one would get $d_{1,2}<0$. However, this doesn't imply that events need to be always ordered in a specific way, as this may depend on the case or the individual.
...
compact in a graph representation. As introduced in the following definition, the total number of time measures implied by a set of $n$ events and the corresponding durations is %therefore 
$n+m=n+n(n-1)/2=n(n+1)/2$. %, which yields a $n+1$-polytope. 
